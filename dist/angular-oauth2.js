...
(function(root, factory) {
    ...
    var ngModule = angular.module("angular-oauth2", [ ngCookies ]).config(oauthConfig).factory("oauthInterceptor", oauthInterceptor).provider("OAuthPassword", OAuthPasswordProvider).provider("OAuthToken", OAuthTokenProvider).provider("OAuthClient", OAuthClientProvider);
        ...
        this.$get = function($http, OAuthToken) {
            var OAuthClient = function() {
                function OAuthClient() {
                    ...
                    if (!config) {
                        throw new Error("`OAuthClientProvider` must be configured first.");
                    }
                }
    ...
    function OAuthPasswordProvider() {
        ...
        this.$get = function($http, OAuthToken) {
            var OAuthPassword = function() {
                function OAuthPassword() {
                    _classCallCheck(this, OAuthPassword);
                    if (!config) {
                        throw new Error("`OAuthPasswordProvider` must be configured first.");
                    }
                }
                _createClass(OAuthPassword, [ {
                    key: "isAuthenticated",
                    value: function isAuthenticated() {
                        return !!OAuthToken.getToken();
                    }
