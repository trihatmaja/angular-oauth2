...
/**
 * Test `OAuthClientProvider`.
 */

describe('OAuthClientProvider', function() {
  var defaults = {
  ...
  describe('configure()', function() {
    ...
    beforeEach(function() {
      angular.module('angular-oauth2.test', [])
        .config(function(OAuthClientProvider) {
  ...
  describe('$get()', function() {
    beforeEach(function() {
      angular.module('angular-oauth2.test', ['angular-cookies.mock'])
        .config(function(OAuthClientProvider) {
          OAuthClientProvider.configure(defaults);
        });
    ...
    describe('isAuthenticated()', function() {
      it('should be true when there is a stored `token` cookie', inject(function(OAuthClient, OAuthToken) {
        OAuthToken.setToken({ token_type: 'bearer', access_token: 'foo', expires_in: 3600, refresh_token: 'bar' });

        OAuthClient.isAuthenticated().should.be.true;
      }));

      it('should be false when there is no stored `token` cookie', inject(function(OAuthClient) {
        OAuthClient.isAuthenticated().should.be.false;
      }));
    });
    ...
    describe('getAccessToken()', function() {
      ...
      it('should call `queryString.stringify`', inject(function(OAuthClient) {
      ...
      it('should return an error if user credentials are invalid', inject(function($httpBackend, OAuthClient) {
      ...
      it('should retrieve and store `token` if request is successful', inject(function($httpBackend, OAuthClient, OAuthToken) {
        $httpBackend.expectPOST(defaults.baseUrl + defaults.grantPath, data)
    ...
    describe('refreshToken()', function() {
      ...
      it('should call `queryString.stringify`', inject(function(OAuthClient, OAuthToken) {
        sinon.spy(queryString, 'stringify');
      ...
      it('should return an error if `refresh_token` is missing', inject(function($httpBackend, OAuthClient) {
        ...
        OAuthClient.getRefreshToken().then(function() {
          should.fail();
        }).catch(function(response) {
          response.status.should.equal(400);
      ...
      it('should return an error if `refresh_token` is invalid', inject(function($httpBackend, OAuthClient, OAuthToken) {
        OAuthToken.setToken({ token_type: 'bearer', access_token: 'foo', expires_in: 3600, refresh_token: 'bar' });
      ...
      it('should retrieve and store `refresh_token` if request is successful', inject(function($httpBackend, OAuthClient, OAuthToken) {
        OAuthToken.setToken({ token_type: 'bearer', access_token: 'foo', expires_in: 3600, refresh_token: 'bar' });

        $httpBackend.expectPOST(defaults.baseUrl + defaults.grantPath, queryString.stringify(data))
          .respond({ token_type: 'bearer', access_token: 'qux', expires_in: 3600, refresh_token: 'biz' });

        OAuthClient.getRefreshToken().then(function(response) {
    ...
    describe('revokeToken()', function () {
      ...
      it('should call `queryString.stringify` with `access_token` if `refresh_token` is not available', inject(function(OAuthClient, OAuthToken) {
        sinon.spy(queryString, 'stringify');
      ...
      it('should return an error if `token` is missing', inject(function($httpBackend, OAuthClient) {
        var data = queryString.stringify({
          client_id: defaults.clientId,
          token: undefined,
          token_type_hint: 'access_token',
        ...
        OAuthClient.revokeToken().then(function() {
          should.fail();
        }).catch(function(response) {
          response.status.should.equal(400);
        });
      ...
      it('should revoke and remove `token` if request is successful', inject(function($httpBackend, OAuthClient, OAuthToken) {
        OAuthToken.setToken({ token_type: 'bearer', access_token: 'foo', expires_in: 3600, refresh_token: 'bar' });
